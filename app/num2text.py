...
def russian_convert_numbers_to_words(text: str) -> str:
    res = _NUMBER_WITH_SEPARATOR_RX.sub(lambda m: m[0].replace(",", ""), text)
    # res = _CURRENCY_RX.sub(lambda m: m[2] + _CURRENCY_MAP.get(m[1], m[1]), res)
    res = _NUMBER_RX.sub(lambda m: num2text(int(m[0])), res)
    return res

if __name__ == "__main__":
    text = [
        "У меня было 50 варенников.",
        "У нас есть 20 долларов.",
        "Я тут 1",
    ]

    for _str in text:
        print(russian_convert_numbers_to_words(_str))